...
router.post('/login', async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const Owner = await OwnerModel.findOne({ email: email, password: password });
...
//add the vehical
router.post('/vehical/:id', upload.single("Image"), async (req: Request, res: Response, next: NextFunction) => {
...
//get the vehicals 
router.get('/vehical/:id', async (req: Request, res: Response, next: NextFunction) => {
    const id: string = req.params.id as string;
...
//update the vehicals
router.put('/vehicals/:ownerid/:id', upload.single("Image"), async (req: Request, res: Response, next: NextFunction) => {
    const id: string = req.params.id as string;
    ...
    try {
        ...
        const updateVehical = await VehicalModel.findByIdAndUpdate(id, {
            vehicalName,
            vehicalNumber,
            vehicalType,
            Amount,
...
//delete the vehical
router.delete('/vehical/:id', async (req: Request, res: Response, next: NextFunction) => {
    const id: string = req.params.id as string;
    try {
        const vehical = await VehicalModel.findById(id)
        if (!vehical) {
...
//bank detils
router.post('/bank/:id', async (req: Request, res: Response, next: NextFunction) => {
    const id: string = req.params.id as string;
    try {
        const Owner = await OwnerModel.findById(id)
        if (!Owner) {
    ...
    try {
       ...
       console.log(vehicleIds,"vehiclasids")
        ...
        const bookings = await BookModel.aggregate([
            {
                $match: {
                    vehical: { $in: vehicleIds.map(id => new mongoose.Types.ObjectId(id)) },
...
router.put('/status/:id',uploads.single('Lisence'),async(req:Request,res:Response,next:NextFunction)=>{
    const id : string = req.params.id as string
    try{
        ...
        if(status === "accept"){
            if (req.file) {
             ...
             update  = await BookModel.findByIdAndUpdate(id,{
                status:status,
...
router.put('/statuss/:id', async (req, res, next) => {
    ...
    try {
      ...
      const update = await BookModel.findByIdAndUpdate(id, { status }, { new: true });
...
//get the payment
router.get('/payment/:id',async(req:Request,res:Response,next:NextFunction)=>{
    const id :string = req.params.id as string;
    try{
        const Owner = await OwnerModel.findById(id)
        if(!Owner){
       ...
       console.log(vehicalsid)
        ...
        const payments = await PaymentModel.aggregate([
            {
                $match:{
                    vehical:{$in:vehicalsid.map((item:any)=>new mongoose.Types.ObjectId(item))}
                }
            },{
                $lookup:{
                    from: VehicalModel.collection.name,
...
router.get('/feedbacks/:id',async(req:Request,res:Response,next:NextFunction)=>{
    ...
    try{
        ...
        const Feedbacks : any[] | null = await FeedBackModel.aggregate([
            ...
            {
                $match:{
                    'orders.vehical':{$in : vehicalsid.map((item:string)=> new mongoose.Types.ObjectId(item))}
                }
            },{
                $lookup:{
                    from:VehicalModel.collection.name,
